...
class GeneticPlayer:
    def __init__(self,pop_size,num_generations,num_trails,window_size,hidden_size,board_size,mutation_chance = 0.1,mutation_size = 0.1):
        ...
        self.pop = [self.generate_brain(self.window_size**2,self.hidden_size,len(game.MOVES))for _ in range(self.pop_size)]

    def generate_brain(self,input_size,hidden_size,output_size):
        # print("generate_brain")
    ...
    def get_move(self,board,snake):
        ...
        return game.MOVES[max_index]

    def proccess_board(self,board,x1,y1):
        # print("proccess_board")
        ...
        for i in range(self.window_size):
            for j in range(self.window_size):
                ...
                if ii<0 or jj<0 or ii>=self.board_size or jj>=self.board_size:
                    input_vector[i][j] = -1
                elif board[ii][jj] == game.FOOD:
                    input_vector[i][j] = 1
                elif board[ii][jj] == game.EMPTY:
                    input_vector[i][j] = 0
                else:#another snake
                    input_vector[i][j] = -1
        if self.display:
    ...
    def reproduce(self,top_25):
        ...
        for _ in range(self.pop_size//2):
            new_pop.append(self.generate_brain(self.window_size**2,self.hidden_size,len(game.MOVES)))
        return new_pop
    ...
    def one_generation(self):
        ...
        for i in range(self.pop_size):
            for j in range(self.num_trails):
                ...
                game_ = game.Game(self.board_size,self)
                outcome = game_.play()
                score = len(game_.snake)
                scores[i] += score
    ...
    def evolve_pop(self):
        ...
        key = input("enter any key to display board")

       
        self.display = True
        self.current_brain = self.pop[0]
        print(self.current_brain)
        game_ = game.Game(self.board_size,self,display=True)
        gui = game.Gui(game_,400) 
        game_.play()
        # print("snake length",len(game_.snake))
